...
export function MapScene({
  ...
  useFrame(({ camera, size }) => {
    ...
    const fitMapHeight =
      geometry.planeHeight / (2 * Math.tan(verticalFov / 2) * usableHeightRatio);
    const fitMapWidth =
      geometry.planeWidth /
    ...
    const desiredDistances = [
      staticFitDistance,
      Math.max(staticFitDistance * 0.62, 6.4),
      Math.max(staticFitDistance * 0.28, 3.2),
    ];
    ...
    const panLimits = [
      { x: 0, z: 0 },
      { x: geometry.planeWidth * 0.26, z: geometry.planeHeight * 0.22 },
      { x: geometry.planeWidth * 0.43, z: geometry.planeHeight * 0.38 },
    ][zoomLevel];
    ...
    if (recentering.current) {
      ...
      controls.current.setAzimuthalAngle(azimuth + shortestTurn * 0.105);
      if (
        Math.abs(shortestTurn) < 0.002 &&
        controls.current.target.length() < 0.01
      ) {
  ...
  const background = config.palette.background ?? config.palette.seaDeep;

  return (
    <>
      <color attach="background" args={[background]} />
      <fog attach="fog" args={[background, 15, 31]} />
      ...
      <MapSea config={config} geometry={geometry} parchment={zoomLevel === 0} />
      <Suspense fallback={<MapLoadingTerrain config={config} geometry={geometry} />}>
        <MapTerrain config={config} geometry={geometry} parchment={zoomLevel === 0} />
      </Suspense>
      ...
      <MapControls
        ...
        enableZoom={false}
        minDistance={3.2}
        maxDistance={Math.max(15.4, geometry.planeWidth * 1.2)}
        minPolarAngle={0.001}
        maxPolarAngle={1.075}
        enableDamping
        dampingFactor={0.08}
